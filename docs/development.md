...
Для рекомендуемого сценария через Docker Compose на хосте нужны:
...
Docker здесь не обязателен: проект можно запустить и в обычном окружении с PHP, Composer, PostgreSQL и веб-сервером. Этот вариант отдельно описан в разделе [«Запуск без Docker»](#запуск-без-docker).
...
| `make test-db-check` | Проверяет имя базы и состояние тестовых данных | Ожидаются 3 статьи с id от 1 до 3 |
...
Команды PHP и Composer, которые создают файлы, выполняются в контейнере от пользователя `app`, связанного с UID/GID пользователя хоста через `.env.docker`.
...
Docker Compose остаётся рекомендуемым и самым воспроизводимым способом запуска. Но если Docker по каким-то причинам не подходит, проект можно поднять и вручную.

Отдельный трактат по установке PHP-FPM, PostgreSQL и настройке Nginx здесь не приводится: раз уж выбран ручной путь, с базовой настройкой этих сервисов, думаю, разберётесь. Чтобы не начинать совсем с пустого листа, в репозитории есть пример конфигурации Nginx, а Docker-конфигурация показывает используемые расширения PHP и параметры окружения.
